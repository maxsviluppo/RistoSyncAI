            ...
            {viewMode === 'active' ? (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 pb-10">
                    {displayedOrders.map((order) => {
                        ...
                        if (isLingering) borderColor = 'border-green-500 shadow-[0_0_30px_rgba(34,197,94,0.4)] bg-green-900/20'; // LINGER STYLE
                        ...
                        return (
                            <div key={order.id} className={`flex flex-col rounded-xl shadow-2xl border-t-8 ${borderColor} bg-slate-800/95 bg-gradient-to-br from-slate-800 to-slate-900 text-slate-200 overflow-hidden relative ${isLingering ? 'animate-pulse' : 'hover:-translate-y-1'} transition-transform`}>
                                <div className={`p-4 border-b border-slate-700/50 flex justify-between items-start bg-slate-800/50`}>
                                    <div>
                                        ...
                                        <span className={`inline-block px-2 py-0.5 rounded text-xs font-bold uppercase mt-1 ${isLingering ? 'bg-green-600 text-white' : getStatusColor(order.status)}`}>
                                            {isLingering ? 'ARCHIVIAZIONE...' : (order.status === OrderStatus.READY ? 'COMPLETA' : order.status)}
                                        </span>
